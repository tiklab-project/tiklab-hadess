 ...
 */
@Service
public class MavenUploadOldServiceImpl implements MavenUploadOldService {
    private static Logger logger = LoggerFactory.getLogger(MavenUploadOldServiceImpl.class);
     ...
     * @return
     */
    public Result proxyInstall( List<RepositoryGroup> libraryList,String relativePath) throws IOException {
        // 过滤代理库信息
        ...
        if (CollectionUtils.isNotEmpty(groupList)){
            ...
            if (CollectionUtils.isNotEmpty(remoteProxyList)){
             ...
             /*   List<String> proxyList = remoteProxyList.stream().map(RepositoryRemoteProxy::getAgencyUrl).collect(Collectors.toList());
                ...
                Result<byte[]> result = callAgencyMaven(relativeAbsoluteUrl);
